...
| 路径 | 内容 |
|---|---|
| `governance/` | 方法论本体：`rules/` 页格式与判据，`flow/` 初始化 / 更新 / 概览 / 出图流程，`PATHS.md` 路径表，`AGENTS.md` 治理入口，`tool-setup.md` 代码查询工具安装 |
...
流程核对规则和代码时要用两个命令行工具：[codegraph](https://github.com/colbymchenry/codegraph)（符号查询）和 [graphify](https://github.com/Graphify-Labs/graphify)（代码聚簇）。**只装命令行，不接进 AI 助手。**安装、建索引、就绪检查见 [`governance/tool-setup.md`](governance/tool-setup.md)。

## 挂到一个项目里

1. 按 [`governance/tool-setup.md`](governance/tool-setup.md) 装好两个工具。
...
5. 先走一次 `governance/flow/project-setup.md`（项目适配，一次性，包含工具就绪检查），之后按 `governance/flow/README.md` 分诊进各条流程。
